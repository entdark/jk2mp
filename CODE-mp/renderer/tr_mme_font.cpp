...
static mmeFontInfo_t registeredFont[MAX_FONTS];

#ifdef BUILD_FREETYPE

typedef struct freeTypeFont_s {
	char fontName[MAX_QPATH];
	mmeFontInfo_t fontInfo;
...
static FT_Bitmap *R_RenderGlyph(FT_GlyphSlot glyph, mmeGlyphInfo_t* glyphOut) {
  ...
  if ( glyph->format == ft_glyph_format_outline ) {
    ...
    bit2 = (FT_Bitmap *)Z_Malloc(sizeof(FT_Bitmap), TAG_FONT);
    ...
    bit2->buffer     = (unsigned char *)Z_Malloc(pitch*height, TAG_FONT);
    bit2->num_grays = 256;
...
/*static void WriteTGA (char *filename, byte *data, int width, int height) {
	...
	buffer = (byte *)Z_Malloc(width*height*4 + 18, TAG_FONT);
...
static mmeGlyphInfo_t *RE_ConstructGlyphInfo(unsigned char *imageOut, int *xOut, int *yOut, int *maxHeight, FT_Face face, const unsigned char c, qboolean calcHeight) {
  int i;
  static mmeGlyphInfo_t glyph;
  unsigned char *src, *dst;
  float scaled_width, scaled_height;
  FT_Bitmap *bitmap = NULL;

  Com_Memset(&glyph, 0, sizeof(mmeGlyphInfo_t));
...
void R_MME_RegisterFont(const char *fontName, int pointSize, mmeFontInfo_t *font) {
  ...
  //Check if we've already loaded this font/pointsize combination
	...
	out = (unsigned char *)Z_Malloc(1024*1024, TAG_FONT);
	if (out == NULL) {
...
//			}
			...
			Com_Memcpy(&font->glyphs[i], glyph, sizeof(mmeGlyphInfo_t));
			i++;
		}
	}
	...
	freeTypeFont = (freeTypeFont_t *)ri.Malloc( sizeof( *freeTypeFont ), TAG_FONT, qfalse);
...
//	if (r_saveFontData->integer) { 
//		ri.FS_WriteFile(va("fonts/fontImage_%i.dat", pointSize), font, sizeof(mmeFontInfo_t));
//	}
	Z_Free(out);
