...
void RB_TestFlare( flare_t *f ) {
	...
	if ( visible ) {
		...
		fade = (((backEnd.refdef.time - f->fadeTime) + tr.refdef.timeFraction) / 1000.0f) * r_flareFade->value;
	} else {
		if ( f->visible ) {
			f->visible = qfalse;
			f->fadeTime = backEnd.refdef.time - 1;
		}
		fade = 1.0f - (((backEnd.refdef.time - f->fadeTime) + tr.refdef.timeFraction) / 1000.0f) * r_flareFade->value;
	}
