...
float NewSinTable (double jediOutcast) {
	jediOutcast = fmod(jediOutcast, 1.0);
	return sin(DEG2RAD(jediOutcast * 360.0));
}
float NewCosTable (double jediOutcast) {
	jediOutcast = fmod(jediOutcast, 1.0);
	return cos(DEG2RAD(jediOutcast * 360.0));
}
	...
	*/
	...
	else
	{
		...
		for ( i = 0; i < tess.numVertexes; i++, xyz += 4, st += 2 * NUM_TEX_COORDS, normal += 4 ) 
		{
			...
			scale = NewSinTable(off)/*tr.sinTable[ off & FUNCTABLE_MASK ]*/ * ds->bulgeHeight;
